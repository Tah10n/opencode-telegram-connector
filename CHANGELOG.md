...
### Added
- `/permissions` can show and switch Codex-like OpenCode permission profiles (`suggest`, `auto-edit`, `full-auto`, and reset) from Telegram private chats, with read-only group views and backed-up local `opencode.json`/`opencode.jsonc` writes. Built-in profiles deny `.env`/`.env.*` content for both read and grep while allowing `.env.example`. `full-auto` asks on unknown future permissions, and explicit permission config paths are constrained to local OpenCode config filenames, local project boundaries, and explicit remote-directory opt-in for same-platform remote paths.
