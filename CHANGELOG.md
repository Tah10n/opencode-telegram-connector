...
- Large-file attachment confirmations are now persisted before Telegram buttons become visible, strictly bound to the original context/project/session/message/update/file identity, and reconciled after restart without repeating a completed download or OpenCode POST.
...
- First-run Telegram backlog skipping now uses one durable server-side tail cutoff; later arrivals are processed by the normal loop, and readiness stays false until recovery state is persisted. Clearly unsent requests may retry the snapshot, but ambiguous responses and interrupted cutoff intent fall back durably to offset `0` without issuing another negative-offset request.
